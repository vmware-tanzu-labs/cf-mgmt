...
var _ = Describe("given OrgManager", func() {
	var (
		fakeOrgClient    *orgfakes.FakeCFOrganizationClient
		fakeDomainClient *orgfakes.FakeCFDomainClient
		fakeJobClient    *orgfakes.FakeCFJobClient
	...
	BeforeEach(func() {
		fakeOrgClient = new(orgfakes.FakeCFOrganizationClient)
		fakeDomainClient = new(orgfakes.FakeCFDomainClient)
		fakeJobClient = new(orgfakes.FakeCFJobClient)
	...
	Context("ClearMetadata()", func() {
		It("should remove metadata from given org", func() {
			org := &resource.Organization{
				GUID:     "org-guid",
				Metadata: resource.NewMetadata().WithLabel("foo", "bar", "qux"),
			}
			err := orgManager.ClearMetadata(org)
			Expect(err).ShouldNot(HaveOccurred())
			Expect(fakeOrgClient.UpdateCallCount()).Should(Equal(1))
			_, orgGUID, orgUpdate := fakeOrgClient.UpdateArgsForCall(0)
			Expect(orgGUID).Should(Equal("org-guid"))
			for k := range orgUpdate.Metadata.Labels {
				Expect(orgUpdate.Metadata.Labels[k]).Should(BeNil())
			}
			for k := range orgUpdate.Metadata.Annotations {
				Expect(orgUpdate.Metadata.Annotations[k]).Should(BeNil())
			}
		})
	})
})
