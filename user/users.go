...
func (m *DefaultManager) SyncUsers(uaaUsers map[string]uaa.User, updateUsersInput UpdateUsersInput) error {
	...
	if err := m.RemoveUsers(uaaUsers, roleUsers, updateUsersInput); err != nil {
		return err
	}
	return nil
}
...
func (m *DefaultManager) SyncSamlUsers(roleUsers map[string]string, uaaUsers map[string]uaa.User, updateUsersInput UpdateUsersInput) error {
	for _, userEmail := range updateUsersInput.SamlUsers {
		...
		if _, ok := roleUsers[lowerUserEmail]; !ok {
			if err := updateUsersInput.AddUser(updateUsersInput, userEmail, m.LdapConfig.Origin); err != nil {
				return err
			}
		} else {
...
func (m *DefaultManager) RemoveUsers(uaaUsers map[string]uaa.User, roleUsers map[string]string, updateUsersInput UpdateUsersInput) error {
	if updateUsersInput.RemoveUsers {
		for roleUser, roleUserGUID := range roleUsers {
			uaaUser, ok := uaaUsers[roleUserGUID]
			if !ok {
				return fmt.Errorf("Unable to find user with id %s and userName %s", roleUserGUID, roleUser)
			}
			if err := updateUsersInput.RemoveUser(updateUsersInput, roleUser, uaaUser.Origin); err != nil {
				return err
			}
		}
	} else {
