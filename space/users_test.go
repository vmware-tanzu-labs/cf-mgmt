...
var _ = XDescribe("given SpaceManager", func() {
	...
	Context("UpdateSpaceUsers()", func() {
		...
		It("update org users where users are already in uaac", func() {
			...
			mockCloudController.EXPECT().GetCFUsers("space1GUID", "space", "developers").Return(make(map[string]string), nil)
			mockCloudController.EXPECT().GetCFUsers("space1GUID", "space", "managers").Return(make(map[string]string), nil)
			mockCloudController.EXPECT().GetCFUsers("space1GUID", "space", "auditors").Return(make(map[string]string), nil)
